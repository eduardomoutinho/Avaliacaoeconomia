...
\`\`\`{r}\| label: dadosempainel #\| warning: false #\| echo: false

ggplot(dadospop %\>% filter(!is.na(populacao_total)), aes(x = year, y = populacao_total, group = country)) + geom_line(alpha = 0.3) + transition_reveal(year) + labs(title = "Ano: {frame_along}", y = "População Total", x = "Ano")
